    ...
    <div class="h-full flex items-center">
        ...
        <nav aria-label="breadcrumb" class="-intro-x h-[45px] mr-auto">
            <ol class="breadcrumb breadcrumb-light">
               ...
               @foreach(explode('.' , \Request::route()->getName()) as $key => $route)
                   @if( $key < count(explode('.' , \Request::route()->getName()))-2)
                    <li class="breadcrumb-item"><a href="">{{ ucwords($route) }}</a></li>
                    @endif
                       @if( $key === count(explode('.' , \Request::route()->getName()))-2)
                           <li class="breadcrumb-item active"><a href="">{{ ucwords($route) }}</a></li>
                       @endif
                        @endforeach
            </ol>
        </nav>
